...
AnalyseSims::AnalyseSims(TTree *chain, const size_t &last_bin)
    ...
    , ocl_labr{LABR_detector_data("OCLLABR_Energy", 0), LABR_detector_data("OCLLABR_Energy", 1), LABR_detector_data("OCLLABR_Energy", 2),
               LABR_detector_data("OCLLABR_Energy", 3), LABR_detector_data("OCLLABR_Energy", 4), LABR_detector_data("OCLLABR_Energy", 5)}
...
size_t AnalyseSims::loop()
{
    ...
    for ( int i = 0 ; i < num_entries ; ++i ){
        ...
        if ( i % 10000 == 0 ){
            std::cout << i*100.0/num_entries << "\r" << std::flush;
        }
