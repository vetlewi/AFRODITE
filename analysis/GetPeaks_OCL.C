...
int GetEnergyFromName(const std::string fname)
{
  // First we will strip anything that isnt needed. Means that we will
  // find the last '/' character and '.root' part.

  std::string just_file_name = fname.substr(fname.find_last_of('/')+1);
  std::string energy_in_keV = just_file_name.substr(fname.find('_')+1, fname.find("keV")-fname.find('_'));
  return std::stoi(energy_in_keV);
}
...
string outdir = "mama_spectra_ocl";
