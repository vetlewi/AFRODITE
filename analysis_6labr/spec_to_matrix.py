...
if __name__ == "__main__":
    ...
    files = subprocess.run(['find mama_spectra/*.m'], shell=True, capture_output=True).stdout
    ...
    for file in files:
        if "ocl" not in file:
            continue
        energy_grid.append(int(file[file.find("ocl_")+4:file.find("keV")]))
    ...
    for i, (energy, nevent) in enumerate(zip(tqdm(energy_grid), nevents)):
        ...
        fn = specdir / f"energy_ocl_{energy}keV_{nevent}entries.m"
        if not fn.exists():
            continue
        ...
        vec.save(f"export/resp_m{i}.m", _assert=False)
        # vec.plot(ax=ax)
